                ...
                {steps.map((step, index) => {
                  ...
                  return (
                    <div key={index} className="relative flex justify-center">
                      ...
                      <div className="relative z-10 flex items-center justify-center w-16 h-16 rounded-full bg-red-500 text-white font-bold text-xl shadow-lg transition-transform duration-300 hover:scale-110 group">
                        {step.number}
                      ...
                      {!isLast && (
                        <div className="absolute left-1/2 top-1/2 transform -translate-y-1/2 flex items-center justify-end" style={{ width: '100%', paddingLeft: '2rem', paddingRight: '2rem' }}>
                          <div className="flex-1 h-1 bg-gradient-to-r from-red-500 to-red-300"></div>
                          <svg className="w-6 h-6 text-red-300 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <div key={index} className="flex flex-col items-center text-center group">
                    {/* Icon Container */}
                    <div className="w-16 h-16 rounded-full bg-white border-2 border-red-100 flex items-center justify-center mb-4 group-hover:border-red-500 group-hover:bg-red-50 transition-all duration-300 shadow-md">
                      {IconComponent ? (
                        <IconComponent
                ...
                <div key={index} className="relative">
                  <div className="flex gap-6">
                    ...
                    <div className="flex flex-col items-center">
                      {/* Number Circle */}
                      <div className="flex items-center justify-center w-14 h-14 rounded-full bg-red-500 text-white font-bold text-lg shadow-lg mb-3">
                        {step.number}
                      </div>

                      {/* Icon */}
                      <div className="w-12 h-12 rounded-full bg-white border-2 border-red-100 flex items-center justify-center shadow-md">
                        {IconComponent ? (
                          <IconComponent
                      ...
                      {/* Connecting Line */}
                      {!isLast && (
                        <div className="w-1 flex-1 bg-gradient-to-b from-red-300 to-red-100 mt-3" style={{ minHeight: '40px' }}></div>
                      )}
                    </div>
